...
实现了一个简易的索引式文件系统，用一个文件来模拟硬盘，通过对该文件的读写来模拟硬盘的读写。然而硬盘中存储的是01二进制数据，但文件中只能以字节为单位进行存储，因此为方便起见，直接在文件中存储字符而非01，其中的1个字节就相当于磁盘上的8 bit。硬盘上的写入方式一般为覆盖式写入，相当于C语言中的`fopen(char *path,"r+")`，另外用`int fseek( FILE *stream, long offset, int origin )`函数来模拟硬盘寻址。
...
+ 删除条目时需要将后面的条目前移，但不能单纯地把后面的内容往前移动`ITEM_SIZE`位，因为块之间并不是连续的，需要考虑将某一块的第一个条目移到前一块的最后一个条目上。
